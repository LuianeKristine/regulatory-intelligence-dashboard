...
:root {
  --bg: #f8f8f7; --surface: #ffffff; --surface-2: #f3f3f2;
  --border: #e4e4e2; --text-1: #1a1a18; --text-2: #6b6b66; --text-3: #a3a39e;
  --gold: #92400e; --gold-bg: #fef3c7; --gold-bd: #fde68a;
  --high: #991b1b; --high-bg: #fef2f2; --high-bd: #fecaca;
  --med: #92400e; --med-bg: #fffbeb; --med-bd: #fde68a;
  --low: #166534; --low-bg: #f0fdf4; --low-bd: #bbf7d0;
  --chg: #1d4ed8; --chg-bg: #eff6ff; --chg-bd: #bfdbfe; --r: 7px;
}
...
section[data-testid="stSidebar"], section[data-testid="stSidebar"] * {
  font-family: 'Inter', sans-serif !important; color: var(--text-1) !important;
}
section[data-testid="stSidebar"] label {
  font-size: 10px !important; font-weight: 600 !important;
  text-transform: uppercase !important; letter-spacing: .1em !important; color: var(--text-3) !important;
}
section[data-testid="stSidebar"] .stTextInput input {
  font-family: 'Inter', sans-serif !important; font-size: 13px !important;
  background: var(--surface-2) !important; border: 1px solid var(--border) !important;
  border-radius: var(--r) !important; color: var(--text-1) !important;
}
...
section[data-testid="stSidebar"] .stButton > button { font-family: 'Inter', sans-serif !important; }
...
[data-testid="stSidebarNavCollapseButton"],button[data-testid="stBaseButton-headerNoPadding"],
button[aria-label="Close sidebar"],button[aria-label="Open sidebar"] { display:none !important; visibility:hidden !important; }
...
def build_tags(row):
    ...
    dt  = clean(row.get("Doc Type", ""), 60)
    out = ""
    if ta  and ta  not in ("-",): out += f' <span class="tag tag-gold">{ta}</span>'
    if dt  and dt  not in ("-",): out += f' <span class="tag">{dt}</span>'
    if src and src not in ("-",): out += f' <span class="tag">{src}</span>'
    return out

def filter_df(df, search="", ta_f=None, pri_f=None):
...
def sort_df(df):
    ...
    if "Priority" in df.columns:
        df["_pri_sort"] = df["Priority"].fillna("").str.strip().str.lower().map(lambda p: pri_order.get(p, 3))
    else:
        df["_pri_sort"] = 3
    ...
    if date_col:
        ...
        df = df.sort_values(["_pri_sort","_date_sort"], ascending=[True,False]).drop(columns=["_pri_sort","_date_sort"])
    else:
        df = df.sort_values("_pri_sort").drop(columns=["_pri_sort"])
    return df
...
def render_card(row, key, show_save=True):
    ...
    st.markdown(
        f'<div class="card card-{pc}">'
        f'<div class="card-hdr"><div class="card-ttl">{ttl}</div>'
        f'<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;flex-shrink:0;">{date_display}</div></div>'
        f'<div class="card-sum">{summary or "No summary available."}</div>'
        f'<div class="card-tags">{tags}{pdf_badge}</div>'
        f'</div>',
        unsafe_allow_html=True
    )
    ...
    if show_save:
        ...
        if st.button("\u2b50 Saved" if already else "\u2606 Save", key=f"sav_{key}", disabled=already):
            ...
            st.session_state.setdefault("pending_favs", []).append({
                ...
                "Published Date": pub, "Priority": pri, "Therapeutic Area": ta,
                "AI Summary": summary,
...
tab_home, tab_fda, tab_appr, tab_adcom, tab_srch, tab_arc_t, tab_fav = st.tabs([
    "\U0001f3e0 Home", "\U0001f1fa\U0001f1f8 FDA", "\U0001f48a Drug Approvals",
    "\U0001f3db\ufe0f Advisory Committees", "\U0001f50d Search", "Archive", "\u2b50 Favorites"
])
...
# ══════════════════════════════════════════════════════════════
# US FDA — lê da aba "FDA" do Sheets
# ══════════════════════════════════════════════════════════════
with tab_fda:
    st.markdown('<div class="sec-hdr">\U0001f1fa\U0001f1f8 FDA Updates & News</div>', unsafe_allow_html=True)
    df_f = sort_df(filter_df(df_fda, search_q, sel_ta, sel_pri))
    st.markdown(f'<div class="sec-count">{len(df_f)} items</div>', unsafe_allow_html=True)
    if df_f.empty:
        st.markdown('<div class="empty">No FDA documents yet. Run the scanner to populate.</div>', unsafe_allow_html=True)
    else:
...
with tab_appr:
    ...
    if df_appr.empty:
        st.markdown('<div class="empty">No drug approvals yet. Run the scanner to populate.</div>', unsafe_allow_html=True)
    else:
        df_af = df_appr.copy()
        if search_q:
            q = search_q.lower()
...
with tab_adcom:
    ...
    if df_adcom.empty:
        st.markdown('<div class="empty">No advisory committee meetings yet.</div>', unsafe_allow_html=True)
    else:
        df_ac = df_adcom.copy()
        if search_q:
            q = search_q.lower()
...
with tab_arc_t:
    ...
    st.markdown(f'<div class="arc-note">&#x1F4E6; {len(df_arc) if not df_arc.empty else 0} items \u00b7 FDA news older than 7 days</div>', unsafe_allow_html=True)
    if df_arc.empty:
        ...
        for i, (_, row) in enumerate(df_af.iterrows()):
            title   = safe_html(clean(row.get("Title",""), 200) or "Untitled")
            url     = str(row.get("URL","")).strip()
            summary = safe_html(clean(row.get("AI Summary", row.get("Summary","")), 300))
            pub     = safe_html(clean(row.get("Published Date", row.get("Date","")), 16))
            src     = safe_html(clean(row.get("Source",""), 60))
            pri     = str(row.get("Priority","")).strip()
            pc      = pclass(pri)
            ttl     = f'<a href="{url}" target="_blank">{title}</a>' if url else title
            tags    = f' <span class="tag">{src}</span>' if src and src not in ("-",) else ""
            st.markdown(f"""<div class="card card-{pc}">
  <div class="card-hdr"><div class="card-ttl">{ttl}</div><span class="card-dt">{pub or "\u2014"}</span></div>
  <div class="card-sum">{summary or "No summary."}</div>
  <div class="card-tags">{tags}</div>
